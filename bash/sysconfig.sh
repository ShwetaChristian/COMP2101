...
function displayhelp {
	echo "Usage: `basename $0` [-h] -- program to display system information.
        where :
             --cpu CPU report only
             --disk Disk Usage report only
             --domain report only on the domain
             -h display help message
             --host show only the host report
             --ipconfig IP addresses report only
             --memory System memory usage report only
             --os OS report only
             --printer Printer report only"
}
